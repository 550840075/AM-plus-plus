    ...
    private fun installMiniPlayerWhenAvailable(
        ...
        navigationRoot.post {
            ...
            if (miniPlayer != null) {
                ...
                ModernXposedRuntime.log("phone frosted-glass mini player installed from navigation root")
...
private object PhoneLiquidGlassStyler {
    ...
    private fun addGlassLayer(container: FrameLayout, surface: GlassSurface) {
        ...
        val blurView = BlurView(context).apply {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
                outlineAmbientShadowColor = Color.argb(60, 0, 0, 0)
                outlineSpotShadowColor = Color.argb(80, 0, 0, 0)
            }
        }
        ...
        for (index in 1 until container.childCount) {
            container.getChildAt(index).translationZ = dp(context, 7).toFloat()
        }
    ...
    private fun glassSurfaceDrawable(context: Context, surface: GlassSurface): Drawable {
        ...
        val fillColor = if (night) {
            Color.argb(118, 28, 28, 32)
        } else {
            if (surface == GlassSurface.MINI_PLAYER) {
                Color.argb(148, 242, 242, 247)
            } else {
                Color.argb(138, 238, 238, 244)
            }
        }
        val strokeColor = if (night) {
            Color.argb(60, 255, 255, 255)
        } else {
            Color.argb(40, 120, 120, 130)
        }
    ...
    private class LiquidSelectionIndicator(context: Context) : View(context) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            fillPaint.color = if (night) {
                Color.argb(100, 60, 60, 68)
            } else {
                Color.argb(120, 255, 255, 255)
            }
            canvas.drawRoundRect(rect, radius, radius, fillPaint)
            rimPaint.color = if (night) {
                Color.argb(50, 255, 255, 255)
            } else {
                Color.argb(35, 140, 140, 150)
            }
